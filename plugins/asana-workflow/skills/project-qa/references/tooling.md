...
## Future: Swappable Tooling

The skill treats the testing tool as a concept, not a hardcoded dependency. The verification step checks for a working tool; the investigation step uses it to observe and interact. When iOS (e.g., Safari Web Inspector) or Android (e.g., Chrome DevTools for Android) tooling becomes available as MCP servers, this reference file gets updated — the SKILL.md flow stays the same.
