...
version: 1.0.0
description: >
  Use when the operator asks to investigate a specific question or problem in a running application —
  triggers include "QA this", "why is X broken", "test this flow", "/qa", or any specific question
  about an app's behavior. Requires a running app (local or remote) and Chrome DevTools MCP.
argument-hint: <url-of-sut>
---

# Project QA

Investigate specific questions and problems in running applications. The operator arrives with a concrete question ("Why is this button disabled?", "The checkout flow fails on step 3"), and this skill uses the testing tool to observe the SUT, reproduce the behavior, and report findings with evidence.
...
**Trigger:** `$ARGUMENTS` is a URL, a question, or empty. Anything that is not `verify:`.

**Input:** A specific question or problem about a running application's behavior.

**Output:** A report with answer, root cause, reproduction steps, and evidence (screenshots, screencasts, console/network traces).
...
- **Testing tool** — Chrome DevTools MCP (see `references/tooling.md`). **Blocking** — cannot proceed without it.
- **SUT** — A running application, local or remote (see `references/sut-discovery.md`). **Blocking** — cannot proceed without it.
...
If `$ARGUMENTS` contains a URL, use it. Otherwise, read CLAUDE.md, package.json, and project docs to infer the SUT. See `references/sut-discovery.md` for discovery sources and inference patterns.

**Never assume.** Present what you found and confirm with the operator:

> "I found `npm run dev` which starts on port 3000. Is this the app to test, or should I use a different URL?"
...
Call `list_pages` to confirm Chrome DevTools MCP is connected. See `references/tooling.md` for verification, setup guidance, and failure handling.
...
Reproduce the reported behavior by observing the SUT. Navigate, interact, check console and network. Trace the root cause. See `references/investigation.md` for techniques and anti-patterns.
...
- If behavior now matches **expected** → **Pass**. Include screenshots/screencasts showing the corrected behavior.
- If behavior still matches **actual** → **Fail**. Include evidence showing the issue persists. The fix didn't work.

### Step 5: Report

See `references/reporting.md` for the full report structure. Every report includes:
...
4. **Evidence** — Screenshots, screen recordings (`experimentalScreencast`), console errors, network traces
...
When invoked by `start-task` for bug tickets, this skill participates in a verify → fix → verify loop:

1. **start-task** invokes project-qa in **investigate** mode with the bug description
2. If bug is **Confirmed**, start-task passes the report to `systematic-debugging`
3. After the fix, start-task re-invokes project-qa in **verify** mode with the original reproduction steps
...
- **`references/tooling.md`** — Testing tool verification, screenshots, screencasts
- **`references/sut-discovery.md`** — Where to look for the SUT, confirmation flow
- **`references/investigation.md`** — Observation techniques, source cross-referencing
- **`references/reporting.md`** — Confidence levels, report structure, evidence
